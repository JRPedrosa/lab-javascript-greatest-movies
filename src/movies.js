...
function howManyMovies(arr) {
  const howMany = arr.filter((obj) => {
    return obj.genre.includes("Drama") && obj.director.includes("Steven Spielberg")
  })
...
function scoresAverage(arr) {
  ...
  average = reduced / arr.length;

  averageNum = Number(average.toFixed(2));
...
function dramaMoviesScore(arr) {
  ...
  const dramaScores = arr.filter((obj) => {
  ...
  for (i = 0; i < dramaScores.length; i++) {
      scores.push(dramaScores[i].score);
  };
  ...
  averageNumDrama = Number(sum.toFixed(2));
  
  return averageNumDrama*/
